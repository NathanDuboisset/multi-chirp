...
def build_leaf(n_classes: int, input_len: int = TARGET_AUDIO_LEN) -> Model:
    ...
    x = GaborConv1D(num_filters=LEAF_NUM_FILTERS, kernel_size=LEAF_KERNEL_SIZE, name="gabor_conv")(inputs)
    x = GaussianPool1D(num_filters=LEAF_NUM_FILTERS, pool_size=LEAF_KERNEL_SIZE, stride=LEAF_STRIDE, name="gauss_pool")(x)
